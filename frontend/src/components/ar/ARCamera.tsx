 ...
 */
const STYLE_IMAGE_OVERRIDES: Record<
  string,
  string
> = {
  "men-curly-top":
...
/* ============================================================
   RECOMMENDATION RESPONSE NORMALIZER
============================================================ */
...
export default function ARTryOnPage() {
  ...
  const [faceAligned, setFaceAligned] =
    useState(false);
  ...
  const [selectedStyle, setSelectedStyle] =
    useState<HairstyleRecommendation | null>(
      null
    );
  ...
  const checkFaceAlignment =
    useCallback(
      (
        ...
        /*
         * The fixed guide occupies approximately
         * the central area of the camera.
         *
  ...
  const captureFrame =
    useCallback(async (): Promise<File | null> => {
      ...
      /*
       * The backend should receive the camera image,
       * not the UI guide.
       */
  ...
  const analyzeCurrentFace =
    useCallback(async () => {
      ...
      try {
        ...
        if (styles.length === 0) {
          console.warn(
            "HairVision: recommendation API returned no styles."
          );
        }
        ...
        /*
         * Select requested hairstyle if
         * the user came from an existing card.
         */
        if (requestedStyleId) {
          const requested =
            styles.find(
          ...
          /*
           * Do NOT automatically activate the
           * first hairstyle.
           *
           * User chooses.
           */
    ...
    useCallback(
      async (
        ...
        try {
          ...
          if (styles.length === 0) {
            console.warn(
              "HairVision: no recommendations returned for",
              analysis.face_shape,
              nextGender
            );
          }
              ...
              {cameraStatus ===
                ...
                selectedStyle && (
                  <HairOverlay2D
                    ...
                    onImageError={() =>
                      setError(
                        "The selected hairstyle image could not be loaded. Check the hairstyle asset in frontend/public/hairstyles."
                      )
                    }
                  />
                )}
            ...
            recommendations.length ===
                        ...
                        <div className="relative aspect-[4/3] overflow-hidden bg-[#111114]">
                          <img
                            ...
                            onError={(
                              ...
                              console.warn(
                                "HairVision: hairstyle image not found:",
                                image
                              );
                            }}
                          />
                          ...
                          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                            <div className="rounded-xl border border-white/10 bg-black/40 px-4 py-2 text-center text-[9px] uppercase tracking-[0.14em] text-white/30 backdrop-blur-md">
                              {style.name}
                            </div>
                          </div>
              ...
              <div className="rounded-[22px] border border-white/10 bg-white/[0.025] p-10 text-center">
                ...
                <div className="mt-1 text-[10px] text-white/25">
                  HairVision could not return styles for this category.
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    stableFramesRef.current = 18;
                    void analyzeCurrentFace();
                  }}
                  disabled={analyzing}
                  className="mt-5 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2.5 text-[9px] font-medium uppercase tracking-[0.14em] text-white/60 transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white disabled:opacity-40"
                >
                  <RefreshCw size={11} />
